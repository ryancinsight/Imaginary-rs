...
pub async fn convert_image_format(
    ...
    let field = match multipart.next_field().await {
        ...
        Ok(None) => {
            return Err(AppError::BadRequest(
                "Missing image file in request".to_string(),
            ))
        }
        Err(e) => {
            return Err(AppError::MultipartError(format!(
    ...
    let name = field
        .file_name()
        .unwrap_or("uploaded_image.tmp")
        .to_string();
    let data = field
        .bytes()
        .await
        .map_err(|e| AppError::MultipartError(format!("Failed to read file data: {}", e)))?;
    info!(
        "Received file for conversion: {} ({} bytes)",
        name,
        data.len()
    );
    ...
    let img = image::open(&temp_path).map_err(|e| {
        AppError::ImageProcessingError(format!("Failed to open image '{:?}': {}", temp_path, e))
    })?;
    ...
    params
        .validate()
        .map_err(|e| AppError::BadRequest(format!("Format params validation error: {}", e)))?;
    ...
    let output_image_format =
        ImageFormat::from_extension(target_format_str).unwrap_or(ImageFormat::Png); // Fallback, though validation should catch bad format strings.

    converted_img
        .save_with_format(&output_path, output_image_format)
        .map_err(|e| {
            AppError::FileSystemError(format!(
                "Failed to save converted image '{:?}': {}",
                output_path, e
            ))
        })?;
