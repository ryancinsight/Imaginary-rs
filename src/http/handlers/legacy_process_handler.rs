...
use crate::storage::{cache_result, check_cached_metadata, get_result}; // Assuming storage brings in check_cached_metadata
...
pub async fn process_image(
        ...
        .map_err(|e| AppError::MultipartError(e.to_string()))?
    {
        ...
        if let Some(cached_path) = check_early_cache(&field, "resize", "100x100").await {
            if cached_path.exists() {
                info!(
                    "Image retrieved from cache (metadata match): {:?}",
                    cached_path
                );
                return Ok(Json(json!({
                    "status": "success",
        ...
        if let Some(content_length) = field.headers().get(header::CONTENT_LENGTH) {
            let size = content_length
                ...
                .parse::<usize>()
                .unwrap_or(0);
            if size > config.server.max_body_size {
                // Use configured max_body_size
        ...
        let img = image::open(&file_path).map_err(|e| {
            AppError::ImageProcessingError(format!("Failed to open image '{:?}': {}", file_path, e))
        })?;
        ...
        if let Some(cached_path) =
            get_result(&file_path, operation_name_cache, operation_params_cache)
        {
            if cached_path.exists() {
                info!(
                    "Image retrieved from cache (post-upload): {:?}",
                    cached_path
                );
                return Ok(Json(json!({
                    "status": "success",
    ...
    Err(AppError::BadRequest(
        "No image field in multipart request".to_string(),
    ))
}
