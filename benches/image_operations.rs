...
use imaginary::image::params::{
    BlurParams, CropParams, FormatConversionParams, GammaParams, ResizeParams, RotateParams,
    SmartCropParams,
};
    ...
    for (width, height, size_name) in sizes {
        ...
        group.bench_with_input(
            ...
            |b, img| {
                ...
                b.iter(|| black_box(resize(black_box(img.clone()), black_box(&params))))
            },
        );
...
fn bench_crop(c: &mut Criterion) {
    ...
    for (crop_width, crop_height, crop_name) in crop_sizes {
        group.bench_with_input(BenchmarkId::new("crop", crop_name), &img, |b, img| {
            ...
            b.iter(|| black_box(crop(black_box(img.clone()), black_box(&params))))
        });
    }
    ...
    for angle in angles {
        group.bench_with_input(
            ...
            |b, img| {
                ...
                b.iter(|| black_box(rotate(black_box(img.clone()), black_box(&params))))
            },
        );
    }
...
fn bench_color_operations(c: &mut Criterion) {
    ...
    group.bench_function("grayscale", |b| {
        b.iter(|| black_box(grayscale(black_box(img.clone()))))
    });

    group.bench_function("adjust_brightness", |b| {
        b.iter(|| black_box(adjust_brightness(black_box(img.clone()), black_box(20))))
    });

    group.bench_function("adjust_contrast", |b| {
        b.iter(|| black_box(adjust_contrast(black_box(img.clone()), black_box(1.2))))
    });

    group.bench_function("gamma", |b| {
        let params = GammaParams { value: 2.2 };
        b.iter(|| black_box(gamma(black_box(img.clone()), black_box(&params))))
    });
...
fn bench_filters(c: &mut Criterion) {
    ...
    group.bench_function("blur", |b| {
        ...
        b.iter(|| black_box(blur(black_box(img.clone()), black_box(&params))))
    });

    group.bench_function("sharpen", |b| {
        b.iter(|| black_box(sharpen(black_box(img.clone()))))
    });

    group.bench_function("flip_vertical", |b| {
        b.iter(|| black_box(flip_vertical(black_box(img.clone()))))
    });

    group.bench_function("flip_horizontal", |b| {
        b.iter(|| black_box(flip_horizontal(black_box(img.clone()))))
    });
        ...
        for quality in &qualities {
            group.bench_with_input(
                ...
                |b, img| {
                    ...
                    b.iter(|| black_box(convert_format(black_box(img.clone()), black_box(&params))))
                },
            );
        }
    }
