...
async fn test_post_cache_control_headers() {
    ...
    let form = reqwest::multipart::Form::new()
        .part("image", reqwest::multipart::Part::bytes(image_bytes).file_name("test.png").mime_str("image/png").unwrap())
        .text("operations", r#"[{"operation": "resize", "params": {"width": 1, "height": 1}}]"#);

    let client = reqwest::Client::new();
    let response = client.post(format!("http://{}/pipeline", addr))
        .multipart(form)
        .send()
        .await
        .unwrap();
    ...
    assert!(cache_control.is_some(), "Cache-Control header missing on POST response");
    assert_eq!(cache_control.unwrap(), "no-store", "Unexpected Cache-Control header for POST");
}
...
async fn test_get_cache_control_headers() {
    ...
    let client = reqwest::Client::new();
    let response = client.get(format!("http://{}/pipeline", addr))
    ...
    let cache_control = response.headers().get("Cache-Control");
    assert!(cache_control.is_some(), "Cache-Control header missing on GET response");
    assert_eq!(
...
async fn test_head_cache_control_headers() {
    ...
    let client = reqwest::Client::new();
    let response = client.head(format!("http://{}/pipeline", addr))
        .query(&[("url", image_url), ("operations", operations)])
        .send()
        .await
        .unwrap();
    ...
    let cache_control = response.headers().get("Cache-Control");
    assert!(cache_control.is_some(), "Cache-Control header missing on HEAD response");
    assert_eq!(
        cache_control.unwrap(),
        "public, max-age=31536000, immutable",
